...
export class SharingManager {
	...
	private static ensurePrivateKeyAvailable(): string {
		...
		if (!privateKey) {
			throw new Error(
				'Privátní klíč není dostupný pro dešifrování sdílených dat. Přihlaste se prosím znovu.'
			);
		}
	...
	static async sharePassword(
		shareData: SharePasswordData
	): Promise<ApiResponse<SharedCredentialResponse>> {
		try {
			// 1. Dešifrovat původní heslo (pouze password pole)
			const credentialResponse = await api.credentials.getCredentialCredentialsCredentialIdGet(
				shareData.credentialId
			);
			...
			const publicKeyResponse = await api.api.getUserPublicKeyApiSharingUsersUserIdPublicKeyGet(
				shareData.recipientUserId
			);
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se sdílet heslo'),
	 ...
	 */
	static async getCredentialSharedUsers(
		credentialId: number
	): Promise<ApiResponse<SharedUserResponse[]>> {
		try {
			const response =
				await api.api.getCredentialSharedUsersApiSharingCredentialCredentialIdUsersGet(
					credentialId
				);
			return { data: response.data };
		} catch (error: unknown) {
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se načíst sdílené uživatele'),
					status: getApiErrorStatus(error) || 500
				}
			};
		}
	 ...
	 */
	static async removeCredentialSharing(
		credentialId: number,
		userId: number
	): Promise<ApiResponse<void>> {
		try {
			await api.api.deleteCredentialSharingApiSharingCredentialCredentialIdUserUserIdDelete(
				credentialId,
				userId
			);
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se zrušit sdílení'),
					status: getApiErrorStatus(error) || 500
				}
			};
		}
	...
	static async getSharedPasswords(params?: {
		...
		try {
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se načíst sdílená hesla'),
					status: getApiErrorStatus(error) || 500
				}
			};
		}
		...
		try {
			...
			if (!encryptionKeyManager.isPrivateKeyAvailable()) {
				return {
					error: {
						detail:
							'Privátní klíč není dostupný. Možná vypršela vaše session, přihlaste se prosím znovu.',
						status: 401
					}
				};
			}
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se dešifrovat sdílené heslo'),
					status: 500
				}
			};
		}
	...
	static async searchUsers(
		query: string
	): Promise<ApiResponse<Array<{ id: number; username: string }>>> {
		try {
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se vyhledat uživatele'),
					status: getApiErrorStatus(error) || 500
				}
			};
		}
	 ...
	 */
	static async updateSharedPassword(
		credentialId: number,
		recipientUserId: number
	): Promise<ApiResponse<SharedCredentialResponse>> {
		try {
			...
			const credentialResponse =
				await api.credentials.getCredentialCredentialsCredentialIdGet(credentialId);
			const credential = credentialResponse.data;
			...
			const publicKeyResponse =
				await api.api.getUserPublicKeyApiSharingUsersUserIdPublicKeyGet(recipientUserId);
			const recipientPublicKey = publicKeyResponse.data.public_key;
			...
			const response =
				await api.api.updateCredentialSharingApiSharingCredentialCredentialIdUserUserIdPut(
					credentialId,
					recipientUserId,
					updateRequest
				);
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se aktualizovat sdílené heslo'),
					status: getApiErrorStatus(error) || 500
				}
			};
		}
