...
export class NotesManager {
	 ...
	 */
	private static async encryptNote(
		title: string,
		content: string
	...
	static async getNotes(params?: {
		...
		try {
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Chyba při načítání poznámek'),
					status: status || 500
				}
			};
		}
	...
	static async decryptNote(note: SecureNote): Promise<ApiResponse<DecryptedNote>> {
		try {
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se dešifrovat poznámku'),
					status: 500
				}
			};
		}
	...
	static async createNote(noteData: NoteCreateData): Promise<ApiResponse<SecureNote>> {
		try {
			const { encryptedTitle, encryptedContent, iv } = await this.encryptNote(
				noteData.title,
				noteData.content
			);
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se vytvořit poznámku'),
	...
	static async updateNote(id: number, noteData: NoteUpdateData): Promise<ApiResponse<SecureNote>> {
		try {
			...
			const response = await api.secureNotes.updateSecureNoteSecureNotesNoteIdPut(
				id,
				secureNoteUpdate
			);
			return { data: response.data };
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se aktualizovat poznámku'),
					status: getApiErrorStatus(error) || 500
				}
			};
		}
	...
	static async deleteNote(id: number): Promise<ApiResponse<void>> {
		try {
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se smazat poznámku'),
					status: getApiErrorStatus(error) || 500
				}
			};
		}
