...
export class PasswordManager {
	...
	static async getPasswords(params?: {
		...
		try {
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Chyba při načítání hesel'),
					status: status || 500
				}
			};
		}
	...
	static async decryptPassword(credential: Credential): Promise<ApiResponse<DecryptedPassword>> {
		try {
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se dešifrovat heslo'),
					status: 500
				}
			};
		}
	...
	static async createPassword(passwordData: PasswordCreateData): Promise<ApiResponse<Credential>> {
		try {
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se vytvořit heslo'),
	...
	static async updatePassword(
		...
		try {
			...
			const response = await api.credentials.updateCredentialCredentialsCredentialIdPut(
				id,
				credentialUpdate
			);

			console.log(
				`Heslo ${id} úspěšně aktualizováno${passwordData.password ? ' (včetně změny hesla)' : ''}`
			);
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se aktualizovat heslo'),
					status: getApiErrorStatus(error) || 500
				}
			};
		}
	...
	static async deletePassword(id: number): Promise<ApiResponse<void>> {
		try {
			...
			return {
				error: {
					detail: getApiErrorDetail(error, 'Nepodařilo se smazat heslo'),
					status: getApiErrorStatus(error) || 500
				}
			};
		}
