...
<div class="space-y-2">
	<label class="block text-sm font-medium text-gray-700" for="category-select"> Kategorie </label>
	...
	{:else}
		<div class="grid max-h-32 grid-cols-2 gap-2 overflow-y-auto" id="category-select">
			{#each categories as category (category.id)}
				<button
					type="button"
					class="flex items-center space-x-2 rounded-md border p-2 text-left transition-colors {isSelected(
						category.id
					)
