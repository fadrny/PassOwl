...
<Modal {open} onClose={handleClose} title="Přidat kategorii">
		...
		<div>
			...
			<div class="grid grid-cols-5 gap-3" id="color-select">
				{#each CATEGORY_COLORS as color (color.value)}
					<button
						type="button"
						class="flex flex-col items-center rounded-md border p-2 transition-colors {formData.color_hex ===
						color.value
							? 'border-indigo-500 bg-indigo-50'
							: 'border-gray-300 hover:bg-gray-50'}"
						onclick={() => (formData.color_hex = color.value)}
					>
						<div class="mb-1 h-6 w-6 rounded-full" style="background-color: {color.value}"></div>
						<span class="text-center text-xs text-gray-700">{color.name}</span>
					</button>
				{/each}
			</div>
		</div>
	...
	{#snippet footer()}
		<div class="flex space-x-3">
			<Button variant="secondary" onclick={handleClose} disabled={loading}>Zrušit</Button>
			<Button variant="primary" onclick={handleSubmit} disabled={loading} {loading}>
