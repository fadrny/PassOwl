<script lang="ts">
	...
	const decryptedPasswordIds = $derived(
		new Set(passwords.filter((p) => p.decryptedPassword !== undefined).map((p) => p.id))
	);

	function handleSort(sortBy: string) {
		const newDirection =
			currentSort.by === sortBy && currentSort.direction === 'asc' ? 'desc' : 'asc';
		...
		<div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
			...
			<div class="flex flex-col gap-3 sm:flex-row">
				...
				<div class="flex items-center gap-2">
					...
					<select
						id="category-filter"
						class="block w-40 rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none sm:text-sm"
		...
		<div class="py-12 text-center">
			...
			<p class="mt-1 text-sm text-gray-500">
				{currentCategoryFilter
					? 'Zkuste vybrat jinou kategorii nebo přidat nové heslo.'
					: 'Začněte přidáním prvního hesla.'}
			</p>
		</div>
	{:else}
					...
					{#each passwords as password (password.id)}
						<tr class="hover:bg-gray-50">
								...
								{#if decryptedPasswordIds.has(password.id)}
									<div class="flex items-center gap-2">
										<span class="rounded bg-gray-100 px-2 py-1 font-mono text-sm"
											>{password.decryptedPassword}</span
										>
										<button
											type="button"
							...
							<td class="px-6 py-4 text-right text-sm font-medium whitespace-nowrap">
								<div class="flex items-center gap-2">
									<Button variant="secondary" size="sm" onclick={() => onEdit?.(password.id)}>
										Upravit
									</Button>
									{#if onShare}
										<Button variant="secondary" size="sm" onclick={() => onShare?.(password.id)}>
											Sdílet
										</Button>
									{/if}
								</div>
				...
				<div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
					<div>
						<p class="text-sm text-gray-700">
							Stránka <span class="font-medium">{currentPage}</span> z
