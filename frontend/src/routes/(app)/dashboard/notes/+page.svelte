...
<div class="space-y-6">
		...
		<div class="rounded-md border border-red-200 bg-red-50 p-4">
			<div class="flex">
				...
				<div class="ml-3">
					...
					<div class="mt-2">
						<Button variant="secondary" size="sm" onclick={loadFirstPage}>Zkusit znovu</Button>
					</div>
				</div>
			</div>
		</div>
	...
	{:else}
		...
		<div class="columns-1 gap-4 space-y-4 md:columns-2 lg:columns-3 xl:columns-4">
			{#each displayItems as item (item.id)}
				...
				{:else if item.type === 'skeleton' || item.type === 'loading'}
					<NoteCardSkeleton variant={item.variant} pulse={item.type === 'loading'} />
				{/if}
			{/each}
		</div>
		...
		{#if hasMorePages}
			<div bind:this={loadMoreTrigger} class="flex justify-center py-8">
				...
				{:else}
					<Button variant="secondary" onclick={loadMoreNotes}>Načíst další poznámky</Button>
				{/if}
			</div>
